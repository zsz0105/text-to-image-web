"""
Vercel Serverless Function - 腾讯混元图像生成
"""
import json
import os
import hashlib
...
def _tencent_hunyuan_image(secret_id, secret_key, prompt, width=1024, height=1024):
    service = 'hunyuan'
    ...
    def do_POST(self):
        ...
        try:
            for _ in range(count):
                url = _tencent_hunyuan_image(secret_id, secret_key, prompt, int(w), int(h))
                if url:
                    images.append(url)
